...
def risk_from_adx(adx_value):
    # diskreta nivåer (robust OOS)
...
for i in range(1, len(df) - 1):
    ...
    for t in open_trades:
        ...
        if exit_price is not None:
            ...
            closed_equity += pnl_r * (initial_equity * t["risk_pct"])
    ...
    for t in open_trades:
        ...
        # fast risk i kronor
        unrealized_pnl += r_open * (initial_equity * t["risk_pct"])
    ...
    current_total_risk = sum(t["risk_pct"] for t in open_trades)
    # -------------------------
    # Signals
    # -------------------------
    ...
    adx_val = row["ADX"]
    risk_pct_trade = risk_from_adx(adx_val)

    # om ADX ej redo, hoppa entries (men låt exits/MTM fortsätta)
    if not np.isfinite(risk_pct_trade):
        risk_pct_trade = base_risk_pct
    ...
    # -------------------------
    if long_signal:
        if current_total_risk + risk_pct_trade > MAX_TOTAL_RISK:
            pass  # risk cap nådd, ta ingen ny trade
        else:
...
if trades_df.empty:
    ...
    trade_rets = trades_df["PnL R"].astype(float) * trades_df["Risk Pct"].astype(float)
    ...
    print(trades_df["Risk Pct"].value_counts().sort_index())
